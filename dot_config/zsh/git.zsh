...
if (( $+functions[compdef] )); then
  for _git_alias _git_command in ${(kv)aliases}; do
    case "$_git_command" in
      git)
        compdef _git "$_git_alias=git"
        ;;
      git\ [[:alnum:]_-]*|command\ git\ [[:alnum:]_-]*)
        _git_words=(${=_git_command})
        _git_subcommand="${_git_words[2]}"
        [[ "$_git_subcommand" == git ]] && _git_subcommand="${_git_words[3]}"
        [[ -n "$_git_subcommand" ]] && compdef _git "$_git_alias=git-$_git_subcommand"
        ;;
      LANG=C\ git\ branch*|gsta\ *|ggu)
        compdef _git "$_git_alias=git-branch"
        ;;
    esac
  done

  compdef _git ggl=git-pull ggpull=git-pull gluc=git-pull glum=git-pull ggpur=git-pull
